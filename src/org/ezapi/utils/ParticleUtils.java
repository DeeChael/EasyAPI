...
public final class ParticleUtils {
    ...
    public static void outline(Location from, Location to, Particle particle, double distance) {
        ...
        for (double i = 0; i < Math.abs(x2 - x1); i += distance) {
            ...
            if (min + i > max) break;
            world.spawnParticle(particle, min + i, y1, z1, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, min + i, y1, z2, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, min + i, y2, z1, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, min + i, y2, z2, 1, 0.0D, 0.0D, 0.0D, 0.0D);
        }
        for (double i = 0; i < Math.abs(z2 - x1); i += distance) {
            double min = Math.min(z1, z2);
            double max = Math.max(z1, z2);
            if (min + i > max) break;
            world.spawnParticle(particle, x1, y1, min + i, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, x2, y1, min + i, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, x1, y2, min + i, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, x2, y2, min + i, 1, 0.0D, 0.0D, 0.0D, 0.0D);
        }
        for (double i = 0; i < Math.abs(y2 - y1); i += distance) {
            double min = Math.min(y1, y2);
            double max = Math.max(y1, y2);
            if (min + i > max) break;
            world.spawnParticle(particle, x1, min + i, z1, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, x2, min + i, z1, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, x1, min + i, z2, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            world.spawnParticle(particle, x2, min + i, z2, 1, 0.0D, 0.0D, 0.0D, 0.0D);
        }
    }
